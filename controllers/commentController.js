...
function canDelete(comment, userId, isAdmin) {
  if (isAdmin) return true; // admin can delete any comment
  if (comment.user_id !== userId) return false; // citizen must be the author
  const diffMinutes =
    (Date.now() - new Date(comment.created_at).getTime()) / 60000;
  return diffMinutes < EDIT_LIMIT_MINUTES;
}

function canEdit(comment, userId) {
  // nobody can edit someone else's comment — not even admin
...
function addCanEditFlag(comment, userId, isAdmin) {
  ...
  plain.can_edit   = userId ? canEdit(comment, userId) : false;
  plain.can_delete = userId ? canDelete(comment, userId, isAdmin) : false; // ← add this
  if (plain.Replies) {
...
const addComment = async (req, res) => {
  try {
    ...
    if (report.status === "hidden") {
      return res
        .status(403)
        .json({ success: false, message: "Laporan tidak menerima komentar" });
    }
    ...
    if (parent_id) {
      ...
      if (!parent || parent.report_id !== parseInt(req.params.id)) {
        return res
          .status(400)
          .json({ success: false, message: "Parent komentar tidak valid" });
      }
    }
...
const deleteComment = async (req, res) => {
  try {
    ...
    const isAdmin = req.user.role === "admin";
    if (!canDelete(comment, req.user.id, isAdmin)) {
...
const editComment = async (req, res) => {
  try {
    ...
    if (!canEdit(comment, req.user.id)) {
