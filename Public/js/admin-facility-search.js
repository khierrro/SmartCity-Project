...
function renderTable(facilities) {
  ...
  if (!facilities.length) {
    tbody.innerHTML = `<tr><td colspan="6" class="text-center py-12 text-gray-400">
      <i class="fa-solid fa-building-circle-xmark text-3xl mb-2 block"></i>Tidak ada fasilitas</td></tr>`;
    return;
  }
  ...
  if (editId) {
    const checkExist = setInterval(() => {
      if (allFacilities.length > 0) {
        ...
        if (facility) editFacility(editId);
        else alert("Fasilitas tidak ditemukan");
